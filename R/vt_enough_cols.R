...
#'
#' \code{vt_enough_cols} - Validates that a .csv file's
...
#' )
#'
#' vt_enough_cols(mtcars, map)
#' str(vt_enough_cols(mtcars, map))
vt_enough_cols <- function(data, map, file.name = NULL){
	...
	ncolsr <- list(
		...
		call = "vt_enough_cols",                        ## function name that was called
		file_name = file.name,
		expected_ncols = length(names(map[["column_level"]][[file.name]])),
		actual_ncols = ncol(data)
	)
	class(ncolsr) <-'vt_enough_cols'
	ncolsr
}


#' Prints a vt_enough_cols Object
#'
#' Prints a vt_enough_cols object
#'
#' @param x A vt_enough_cols object.
#' @param \ldots ignored.
#' @method print vt_enough_cols
#' @export
print.vt_enough_cols <- function(x, ...){
