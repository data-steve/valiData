...
vc_missing <- function(data, x, prop.acceptable = 0,
    missing = c("", "NULL", "NA", "N/A", "na", "n/a"), required = FALSE, ...){
	...
	if (!are_valid){
		message <- sprintf(
			"%s is missing %s%% of observations.  This is %s the accepted level of %s%%.\nThe following rows are missing:\n\n%s\n\n\n\n",
			sQuote(x),
